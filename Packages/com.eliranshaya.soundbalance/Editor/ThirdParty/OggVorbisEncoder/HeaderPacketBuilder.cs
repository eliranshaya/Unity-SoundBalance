...
public static class HeaderPacketBuilder
{
    ...
    private static void PackStaticBook(EncodeBuffer buffer, IStaticCodeBook book)
    {
        ...
        if ((book.MapType != CodeBookMapType.Implicit) && (book.MapType != CodeBookMapType.Listed))
            throw new InvalidOperationException($"Unknown {nameof(CodeBookMapType)}: {book.MapType}");

        if (book.QuantList == null)
            throw new InvalidOperationException($"{nameof(book.QuantList)} cannot be null");
